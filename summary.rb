...
if total_failures > 0
  ...
  failed_examples.each do |example, seed|
    ...
    exmaple_link_url = "#{blob_url}/#{example["file_path"].delete_prefix("./")}#L#{example["line_number"]}"
...
failed_examples.first(5).each do |example, seed|
  ...
  exmaple_url = "#{blob_url}/#{file_path}#L#{line_number}"
  exception_class = example.dig("exception", "class") || "UnknownError"
  message = example.dig("exception", "message") || ""
  # Clean up message for Slack code block
  message = message.gsub(/\e\[[0-9;]*m/, "").gsub("`", "").strip

  slack_message += "• <#{exmaple_url}|#{file_path}:#{line_number}>"
  slack_message += " --seed #{seed}" unless all_same_seed
  slack_message += "\n```#{exception_class}\n#{message}```\n"
end

if failed_examples.size > 5
  slack_message += "• And #{failed_examples.size - 5} more failures...\n"
end
